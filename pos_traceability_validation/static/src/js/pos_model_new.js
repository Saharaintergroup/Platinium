odoo.define('point_of_sale.NewProductScreen', function(require) {
    'use strict';
        ...
        async _clickProduct(event) {
            ...
            if (['serial', 'lot'].includes(product.tracking) && (this.env.pos.picking_type.use_create_lots || this.env.pos.picking_type.use_existing_lots)) {
                ...
                if (confirmed) {
                    ...
                    var len = newPackLotLines.length

                    for (var i = 0; i < len; i++) {
                        var pack_line = newPackLotLines[i].lot_name;
                        lot_names.push(pack_line);
                  }
                  const rpcPromise = new Promise((resolve, reject) => {
                    rpc.query({
                        ...
                        if(result != true){
                            if(result[0] == 'error'){
                                alert("this serial "+"["+ result[1]+"]"+" is not belong to this product " )
                            }
                            else if(result[0] == 'stock'){
                                alert("Insufficient quantity for this serial " + "["+result[1]+"]")
                            }
                            resolve(true)
                        }
                        else{
